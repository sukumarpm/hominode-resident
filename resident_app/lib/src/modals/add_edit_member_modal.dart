...
class AddEditMemberModal extends StatefulWidget {
  ...
  const AddEditMemberModal({super.key, this.member, required this.onSave});

  /// Static helper to show the modal
  static Future<void> show(
    BuildContext context, {
    FamilyMember? member,
    required Function(FamilyMember) onSave,
  }) {
    return showDialog(
      context: context,
      barrierDismissible: false,
      barrierColor: Colors.black.withOpacity(0.35),
      builder: (context) => AddEditMemberModal(member: member, onSave: onSave),
    );
  }
...
class _AddEditMemberModalState extends State<AddEditMemberModal>
  ...
  bool _validate() {
    ...
    setState(() {
      ...
      if (_relationController.text.trim().isEmpty) {
        _relationError = 'Relation is required';
        isValid = false;
      } else {
        _relationError = null;
      }
      ...
      if (_ageController.text.trim().isEmpty) {
        ...
        _ageError = 'Age must be greater than 0';
        isValid = false;
      } else {
        _ageError = null;
      }
  ...
  /// Validation helper function (unit-testable)
  static bool validateFamilyMember(FamilyMember member) {
    return member.name.trim().isNotEmpty &&
          ...
          onChanged: (value) {
            ...
            if (error != null) {
              setState(() {
                ...
                if (label == 'Relation') _relationError = null;
                if (label == 'Age') _ageError = null;
              });
            }
          },
